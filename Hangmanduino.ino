...
// This code has been updated to no longer use the pot to select a letter but rather use a toy laptop keyboard.
// You will need the Keyboard library located @ https://github.com/dadecoza/Keyboard to compile this code.
// Updated By: Johannes le Roux
// Date: 12/02/2014
...
void setup() {
  ...
  kb.getChar(); //sneaky trick to randomize without analog pin
...
void loop(){
  ...
  if (alreadyGuessed == 1) {
    ...
    for (int i = 0; i < wordSize; i++) {
      if (guessWord.substring(i, i+1)[0] == guessLetter) {
        lcd.setCursor(i+3,1);
